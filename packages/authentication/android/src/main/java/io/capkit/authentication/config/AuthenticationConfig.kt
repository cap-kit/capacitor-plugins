package io.capkit.authentication.config
 ...
 */
@Serializable
private data class AuthenticationConfigData(
  @SerialName("verboseLogging")
  val verboseLogging: Boolean = false,
)

/**
 * Plugin configuration holder for the Authentication plugin.
 *
 * This class is responsible for reading and parsing static configuration values
 * defined under the `plugins.Authentication` key in `capacitor.config.ts`.
 *
 * Architectural rules:
...
class AuthenticationConfig(
  ...
  /**
   * Raw `plugins.Authentication` configuration JSON from the Capacitor bridge.
   * Consumed by [io.capkit.authentication.AuthenticationImpl.initialize] to
   * resolve the typed provider configuration (e.g. the `google` sub-object).
   */
  val rawConfigJson: String? = plugin.config.getConfigJSON()?.toString()
  ...
  private companion object {
    ...
    private fun parseConfig(plugin: Plugin): AuthenticationConfigData =
      try {
        val configJsonObject = plugin.config.getConfigJSON()
        if (configJsonObject != null) {
          json.decodeFromString<AuthenticationConfigData>(configJsonObject.toString())
        } else {
