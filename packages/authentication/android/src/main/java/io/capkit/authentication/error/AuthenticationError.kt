package io.capkit.authentication.error

/**
 * Native error model for the Authentication plugin (Android).
 *
 * Architectural rules:
...
sealed class AuthenticationError(
  ...
  /**
   * The user dismissed or cancelled the interactive consent flow while it was
   * in progress. Maps to the 'USER_CANCELLED' error code in JavaScript, which
   * must be reported identically across Web, iOS and Android.
   */
  class UserCancelled(
    val msg: String,
  ) : AuthenticationError(msg)
